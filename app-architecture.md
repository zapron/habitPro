...
- **Known scalability gap, not yet fixed (flagged 2026-09-10):** the legacy full-pull path in `pullFromSupabase()` (`sync.ts`) fetches `habits` and `mini_missions` with `.eq("user_id", userId)` and **no `.limit()`/pagination at all** — every cold-start/sign-in hydrate, and every focus-refresh that can't use the delta path, downloads the user's *entire* historical mini-mission and habit rows (full `completion_memory`/`streak_memories` JSON blobs included) into memory and into the persisted Zustand store. Contrast with the offset/limit `PageRequest`/`PageResult` pattern (`src/types/paging.ts`) already used for streak-repair voter lists, group challenges, and live-squad listings — that pattern is not applied here. Client-side rendering is fine regardless (`app/mini/index.tsx`'s four tabs use `FlashList`, which virtualizes render regardless of list length), but the underlying fetch/storage cost grows unbounded with account age. Worth addressing before this becomes a real cold-start/bandwidth problem for long-lived accounts.
...
**Gotcha (found 2026-09-10): the lesson above has two separate call sites, and
missing either one still loses data.** Building freeform capture
(`MiniMission.draftMemories`, same local-only shape as `draftTasks` — see Mini
Missions section) added the preserve-on-merge check to
`mergeDirtyLocalIntoRemote()` (sign-in hydrate) but missed the *other* one,
`useRemoteStoreRefreshOnFocus.ts`'s `preserveLocalMiniProgress()` — a
completely separate function that runs on every mission-screen focus, not just
sign-in. Since that function already had a `draftTasks` branch (added
2026-08-14) but no matching `draftMemories` branch, every background
focus-refresh pull during an active freeform run silently wiped locked-but-
not-yet-completed moments — user-reported as memories vanishing repeatedly
over a single long (8h) Live Squad run, on both iOS and Android. Both
functions now carry both fields forward. **When adding a new local-only field
to `MiniMission`/`Habit`, grep for every existing local-only field's name
(`draftTasks` is the known one) and add a matching branch everywhere that name
already appears — do not assume one preserve-on-merge fix covers both pull
paths.**
...
**Freeform capture (added 2026-09-08)**: `MiniMission.captureMode: "checklist" | "freeform" | undefined`, mutually exclusive with a non-empty `taskChecklist` (enforced at the create-screen UI level, not the type level) — mini missions only, deliberately not ported to Habits. No predefined task list; during the run the user locks as many self-declared "moments" (photo/note) as they want via `src/components/MiniFreeformSheet.tsx`, held in the same local-only, not-synced shape as checklist's `draftTasks` (`MiniMission.draftMemories: StreakMemoryTaskEntry[]`, array instead of a keyed record since there's no fixed task to key against). At completion, the kept entries populate the same `StreakMemory.tasks` array checklist missions use, so every downstream display/Community/gallery consumer works with zero additional branching. Backend: `mini_missions.capture_mode` column, threaded through `rpc_sync_dirty_state`'s explicit column list (see Sync Architecture's gotcha above — this field hit the exact same silent-drop trap on its first pass). Live Squad propagation (added 2026-09-08, see Live Mini Squads section): `live_mini_squads.capture_mode` snapshots the creator's choice so an invite-accepting participant's own local mission also gets `captureMode: "freeform"`, mirroring the pre-existing `task_checklist` snapshot mechanism exactly.
...
- **Capture-mode inheritance on accept (added 2026-09-08).** `live_mini_squads.capture_mode` snapshots the creator's `MiniMission.captureMode` at squad-creation time, the same mechanism `task_checklist` already used (both are `p_task_checklist`/`p_capture_mode` params on `rpc_create_live_mini_squad`/`_v2`, drop-then-recreate on each change to avoid overload ambiguity on a named-parameter call). The accept-invite RPCs need no matching change — inheritance happens client-side, by `app/live-mini/[id].tsx`'s `handleAccept` reading `squad.task_checklist`/`squad.capture_mode` straight off the snapshot JSON returned by `_hp_live_mini_snapshot_json` (which returns `to_jsonb()` of the whole squad row, so any new column rides along automatically once it exists — no RPC change needed there either).
- **Memory gallery perf (fixed 2026-09-10).** Each participant's freeform-moment strip was a plain `ScrollView` + `.map()` — every tile eagerly mounted, nested inside the screen's outer vertical `ScrollView`. With ~10 photos per participant this caused severe frame drops while dragging (user-reported: "पूरा screen एकदम stuck सा हो गया"). Switched to a horizontal `FlatList` with `getItemLayout` (fixed-width tiles, no measurement pass) plus `nestedScrollEnabled`, and wrapped `ParticipantCard` in `memo` (it wasn't, unlike sibling components in the same file) so the once-a-second `liveNowMs` timer tick doesn't force every participant's full card — including its photo strip — to re-render.
- **Tapped gallery tile lost its note when a photo was present (fixed 2026-09-10).** The tap handler branched on whether the tile had a photo: with one, it routed through a plain `openImageUri` state that has no note/label field at all, discarding both; without one, it correctly showed a text card with the note. Unified into one `openGalleryTile` state carrying `{label, note, uri}` always, so an enlarged photo now shows its note as a caption underneath instead of silently dropping it.
...
- A horizontal photo strip inside a per-list-item card (e.g. Live Mini's per-participant memory gallery) should be a `FlatList` with `getItemLayout`, not a `ScrollView` + `.map()` — un-virtualized image tiles nested inside an outer vertical scroll container is a repeatable source of frame drops once a strip has more than a handful of items (found 2026-09-10, see Live Mini Squads section).
- A list-item component that receives a prop which changes on a fast interval shared by the whole list (e.g. a ticking `liveNowMs` passed to every card) should be wrapped in `memo` — otherwise every item re-renders every tick regardless of whether that item's own visible state actually changed (same 2026-09-10 fix, `ParticipantCard`).
...
- **`maybeCompressImageForUpload` (`src/lib/streakMemoryStorage.ts`) can silently upload a full, uncompressed camera original.** It resizes to 1280px width + JPEG quality 0.82 via `expo-image-manipulator`, but the resize step can throw on some Android `content://` URIs; before 2026-07-24 the `catch` had no retry and no logging, silently falling back to the raw picked file (several MB). Confirmed via direct Supabase Storage query: 16% of uploaded files were over 1 MB and accounted for 52% of total bucket bytes. Fixed with a compress-only retry tier plus `console.warn` logging on both failure tiers — but if Storage usage climbs unexpectedly again, check these logs first before assuming the compression settings themselves need changing.
  - **Update (2026-09-05): the fallback tier itself is still not actually fixed, and its residue re-accumulated.** A fresh live query found the free-tier `streak-memories` bucket at 730MB/1GB (73%), with 161 of 1646 files (275MB, 38% of all bytes) sitting well above the 1MB working-as-designed ceiling — the same class of fallback-to-original file, just a fresh batch. Cleaned up via a one-off script (`scripts/compact-streak-memories.mjs`, untracked, not part of the app — needs `SUPABASE_SERVICE_ROLE_KEY`, never the app's own `.env`) that recompresses exactly those paths in place; verified independently via direct SQL query afterward: 730MB → 485MB, ~245MB reclaimed, 0 content lost. **This was a cleanup, not a fix** — `maybeCompressImageForUpload`'s last-resort branch (`readImageBytesForUpload` called on the raw `localUri` when both resize and compress-only throw) still uploads at full original size with no cap. The actual fix — a hard upload-size ceiling, or a more aggressive last-resort compression attempt instead of giving up to raw bytes — is still not written. Expect this bucket to slowly re-bloat until that's done.
...
- **A tap handler that branches into two different viewer states based on "does this item have a photo" can silently drop the fields the losing branch's state type doesn't carry.** Found in Live Mini's gallery-tile viewer (`app/live-mini/[id].tsx`, 2026-09-10): tapping a tile with a photo set a plain `openImageUri: string` state (image-only, no note/label field), while tapping a text-only tile set a separate `openGalleryTile: {label, note}` state — so a photo tile's note was thrown away before render even had a chance to show it, not a rendering bug. User-reported as "the memory note isn't showing when I tap the photo to enlarge it." Fixed by carrying `uri` inside the same `openGalleryTile` state (`{label, note, uri}`) always, and branching only in the *render*, not the tap handler, on whether `uri` is present. When two related pieces of data (here: a tile's photo and its note) can both exist on the same item, don't split them across two state variables with different shapes just because one branch of a click handler "doesn't need" one of the fields.
