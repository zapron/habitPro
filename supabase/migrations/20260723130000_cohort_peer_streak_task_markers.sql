-- Fix: the squad "streak dots" row (CohortPeerStreakDots.tsx) never detected
-- checklist-day photos/notes at all, so tapping a dot for a checklist day always
-- showed "Day marked complete" instead of even attempting to open the memory.
--
-- Root cause: rpc_cohort_peer_habits_v1 and rpc_challenge_streak_members_page_v1
-- both precompute a per-day `streak_memory_markers` map (hasPhoto/hasNote/
-- checkInOnly) that the client uses to decide, client-side, whether a dot tap is
-- worth fetching full detail for. Both functions only ever looked at the legacy
-- top-level streak_memories[date].imageUrl/imageUri/note fields. Checklist-mission
-- days never write those (see 20260723120000_challenge_memory_detail_task_gallery.sql
-- for the identical gap in rpc_challenge_memory_detail_v1) — only
-- streak_memories[date].tasks. So every checklist day's marker came back with
-- hasPhoto=false, hasNote=false, and the client's dot handler short-circuited
-- straight to the "check-in only" modal without ever calling
-- rpc_challenge_memory_detail_v1 (which, as of the migration above, would have
-- returned the task gallery correctly).
--
-- Fix is additive: hasPhoto/hasNote now also look inside streak_memories[date].tasks
-- (a synced/https task photo counts as hasPhoto; any task note counts as hasNote),
-- and the WHERE clause that decides which days get a marker entry at all now also
-- includes days with a non-empty tasks array. Classic single-memory missions have no
-- `tasks` field, so every new clause here is a no-op for them.
