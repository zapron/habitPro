-- Fix: rpc_challenge_memory_detail_v1's `tasks` gallery still silently dropped a task
-- that was logged with neither a photo nor a note — i.e. the user opened the task and
-- tapped through with nothing attached ("bare mark complete"). The previous fix
-- (20260723140000_challenge_memory_detail_text_only_tasks.sql) added note-only tasks
-- but kept requiring "photo OR note", so a bare task still had nothing to match.
--
-- Every element of `tasks` already represents a deliberate user action — the client
-- only ever appends an entry when the user opens that task and commits (see
-- `handleTaskMemoryCommit` in app/habit/[id].tsx), whether or not they attached
-- anything. A task never opened at all simply isn't in the array. So the correct fix
-- is to stop filtering by content entirely: include every logged task. The carousel
-- client (app/challenge-memory.tsx) already falls back to the task's own label when
-- both `imageUrl` and `note` are null, so a bare task renders as its own text slide
