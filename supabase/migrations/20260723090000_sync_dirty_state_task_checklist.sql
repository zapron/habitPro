-- Fix: rpc_sync_dirty_state silently dropped `task_checklist` on every habit push.
--
-- Root cause: this RPC parses the client's JSON payload via
-- jsonb_to_recordset(p_dirty_habits) as x(...) with an explicit column list that
-- predates the task_checklist column (added in
-- 20260722181133_add_task_checklist_and_memory_gallery.sql). Extra JSON fields not
-- named in that list are silently discarded by jsonb_to_recordset — no error, the
-- client sends the field correctly, the server just never wrote it. This is why a
-- freshly created checklist mission always ended up with task_checklist = null on
-- the server (confirmed directly against production data), even though the local
-- app state had it right up until the next sync pull overwrote it with that null.
--
-- Fix is additive to the RPC's behavior: task_checklist is added to the recordset
-- column list, the insert column list/values, and the on-conflict update set. No
-- other column or behavior changes.
