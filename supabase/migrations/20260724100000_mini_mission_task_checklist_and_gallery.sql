-- Phase 0 of docs/MINI_MISSION_CATALOG_ARCHITECTURE.md — extends the multi-task
-- checklist/photo-catalog pattern (already shipped for main missions, see
-- docs/CATALOG_ARCHITECTURE.md) to Mini Missions and Live Squad.
--
-- Additive only. No existing row, RPC caller, or app version is affected:
-- nothing reads or writes these columns/params yet.

alter table public.mini_missions
  add column if not exists task_checklist jsonb;

comment on column public.mini_missions.task_checklist is
  'Optional ordered list of sub-tasks for this mini mission: [{id, label, order}]. Null/empty means the mini mission uses the classic single note+photo completion memory flow unchanged.';

alter table public.live_mini_participants
  add column if not exists memory_gallery jsonb;

comment on column public.live_mini_participants.memory_gallery is
  'Optional per-task photo catalog for this participant''s completion memory: [{taskId, label, note, imageUrl}]. When present, clients should render it as a swipeable gallery; memory_note/memory_image_url stay populated as the cover/first entry for backward compatibility.';

-- rpc_sync_live_mini_progress gains an optional p_memory_gallery param. The
-- prior 8-arg signature is dropped first (rather than relying on
-- create-or-replace) so Postgres doesn't end up with two overloads that could
-- ambiguously resolve a named-parameter call; existing positional callers
-- (all 8 prior args) keep working unchanged against the new 9-arg function,
-- and p_memory_gallery defaults to null so the single-photo path is untouched.
drop function if exists public.rpc_sync_live_mini_progress(
  uuid, text, text, integer, timestamptz, timestamptz, text, text
);
...
begin
  ...
  set
    ...
    memory_gallery = case
      when v_final_status = 'completed' then coalesce(p_memory_gallery, memory_gallery)
      else memory_gallery
    end
  where id = v_participant.id;
