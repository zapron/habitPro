-- Fixes streak-repair prompts wrongly appearing for days BEFORE a user joined an
-- existing group challenge mid-way (e.g. joining a 30-day mission on day 10 and
-- completing that first day used to surface "you missed day 9" — a day the user
-- was never even a participant for). Root cause: habits.start_date is the cohort's
-- shared day-1 anchor (needed so every member's day numbering lines up), not this
-- participant's own tracking start. This column records the latter, set only when
-- accepting an invite to an existing challenge (see app/(tabs)/compete.tsx
-- handleAcceptGroupInvite); null for the challenge creator and for solo/manual habits.
-- src/utils/streakRepairEligibility.ts refuses to offer a repair for any date before it.

alter table public.habits
  add column if not exists joined_challenge_at timestamptz;

comment on column public.habits.joined_challenge_at is
  'When this device joined an existing group challenge mid-way (accept-invite, not creation). Null for the creator and for solo/manual habits. Used to stop streak-repair from ever treating a pre-join day as missed.';

-- rpc_sync_dirty_state's p_dirty_habits branch parses the client payload via
-- jsonb_to_recordset(...) with an explicit column list — any field not named there is
-- silently dropped (this exact bug class already bit task_checklist twice, see
-- 20260723090000_sync_dirty_state_task_checklist.sql and
-- 20260724110000_sync_dirty_state_mini_task_checklist.sql). Adding the column and the
-- RPC fix in the same migration this time instead of two-stepping it.
--
-- Body copied verbatim from 20260724110000_sync_dirty_state_mini_task_checklist.sql
-- aside from adding joined_challenge_at to the habits recordset/insert/update-set.
-- Mini mission handling is completely unchanged.
