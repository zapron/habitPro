-- Fix: rpc_challenge_memory_detail_v1's `tasks` gallery silently dropped any task
-- logged with only a note and no photo.
--
-- Root cause: the gallery-building query only kept rows where
-- `proofUrls[0] like 'http%'` — a synced photo was required to appear at all. A
-- squad member's task logged with just a note (e.g. "drank water — felt great")
-- and no photo never showed up anywhere in the squad-facing carousel, even though
-- the day's marker correctly showed "has memory" (fixed in
-- 20260723130000_cohort_peer_streak_task_markers.sql).
--
-- Fix: a task is now included in `tasks` if it has a synced photo OR a note.
-- `imageUrl` is null for note-only entries — the client renders those as a
-- text-card slide instead of a photo slide. Cover-field selection (the legacy
-- top-level `note`/`imageUrl`, kept for backward compatibility) now explicitly
-- prefers the first task that has a synced photo for `imageUrl`, and falls back to
-- any task's note for `note` — previously it just used array-index 0, which could
-- have been a text-only task and left `imageUrl` unset even when a later task in
-- the array had a photo.
      ...
      if jsonb_typeof(v_tasks) = 'array' and jsonb_array_length(v_tasks) > 0 then
        select jsonb_agg(
                 jsonb_build_object(
                   ...
                   'imageUrl',
                     case when coalesce(t -> 'proofUrls' ->> 0, '') like 'http%'
                       then t -> 'proofUrls' ->> 0
                       else null
                     end,
                   'loggedAt', t ->> 'loggedAt'
        ...
        where coalesce(t -> 'proofUrls' ->> 0, '') like 'http%'
           or nullif(trim(coalesce(t ->> 'note', '')), '') is not null;

        if v_task_gallery is not null and jsonb_array_length(v_task_gallery) > 0 then
          -- Cover fields (legacy top-level note/imageUrl): prefer the first task
          -- that actually has a synced photo; fall back to the first task's note.
          select elem
            into v_cover_task
          from jsonb_array_elements(v_task_gallery) elem
          where elem ->> 'imageUrl' is not null
          limit 1;

          if v_cover_task is not null then
