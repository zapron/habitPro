-- When streak repair is applied (solo or squad), merge a text streak memory so gallery + cohort see it.
-- Conflict: if the day already has user-authored note/media/check-in-only, only add repairSource.

create or replace function public._rpc_merge_repair_streak_memory(
  p_user_id uuid,
  p_habit_id text,
  p_date_str text,
  p_source text
...
declare
  ...
  v_note text;
  v_created text;
  v_has_user_content boolean;
  v_existing_note text;
begin
  if p_source not in ('squad', 'solo') then
    raise exception 'invalid_repair_source';
  end if;

  v_note := case when p_source = 'squad' then 'Streak saved by squad.' else 'Streak repaired.' end;
  v_created := to_char((clock_timestamp() at time zone 'utc'), 'YYYY-MM-DD"T"HH24:MI:SS.US"') || 'Z';

  select coalesce(h.streak_memories, '{}'::jsonb)
  into v_mem
  ...
  if v_day is null or jsonb_typeof(v_day) = 'null' then
    v_mem := jsonb_set(
      ...
      jsonb_build_object(
        'note', v_note,
        'createdAt', v_created,
        'repairSource', p_source
      ),
      true
    );
  else
    v_existing_note := trim(coalesce(v_day ->> 'note', ''));
    v_has_user_content :=
      (coalesce(trim(v_day ->> 'imageUrl'), '') <> '')
      or (coalesce(trim(v_day ->> 'imageUri'), '') <> '')
      or (coalesce(v_day ->> 'checkInOnly', '') = 'true')
      or (
        v_existing_note <> ''
        and v_existing_note not in ('Streak saved by squad.', 'Streak repaired.')
      );

    if v_has_user_content then
      v_mem := jsonb_set(
        v_mem,
        array[p_date_str],
        v_day || jsonb_build_object('repairSource', p_source),
        true
      );
    else
      v_mem := jsonb_set(
        ...
        jsonb_build_object(
          'note', v_note,
          'createdAt', coalesce(nullif(trim(v_day ->> 'createdAt'), ''), v_created),
          'repairSource', p_source
        )
        || case
          when coalesce(trim(v_day ->> 'imageUrl'), '') <> '' then jsonb_build_object('imageUrl', v_day ->> 'imageUrl')
          else '{}'::jsonb
        end
        || case
          when coalesce(trim(v_day ->> 'imageUri'), '') <> '' then jsonb_build_object('imageUri', v_day ->> 'imageUri')
          else '{}'::jsonb
        end,
        true
      );
  ...
  set streak_memories = v_mem,
      updated_at = now()
  where user_id = p_user_id and id = p_habit_id;
end;
$$;

revoke all on function public._rpc_merge_repair_streak_memory(uuid, text, text, text) from public;

-- Keep rpc_request_streak_repair in sync with 20260426074556 (requester_username) + merge memory on solo apply.
...
-- Squad apply: merge memory after completed_dates append.
