...
export function StreakMemoryGallery({ entries }: StreakMemoryGalleryProps) {
    ...
    <>
      <View style={styles.section}>
        <View style={styles.sectionHead}>
          ...
          <Text style={[styles.sectionTitle, { color: theme.colors.textPrimary }]}>Your moments</Text>
        </View>
        <Text style={[styles.sectionHint, { color: theme.colors.textMuted }]}>
          Tap a card to revisit. Moments are view-only after you save them.
        </Text>
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.row}>
          {entries.map(({ dateStr, memory }) => (
            ...
            >
              {memory.imageUri ? (
                <Image source={{ uri: memory.imageUri }} style={styles.thumb} resizeMode="cover" />
              ) : (
        ...
        <Pressable style={[styles.viewerBackdrop, { backgroundColor: "rgba(0,0,0,0.85)" }]} onPress={() => setOpen(null)}>
          <Pressable style={styles.viewerInner} onPress={(e) => e.stopPropagation()}>
            {open?.memory.imageUri ? (
              <Image source={{ uri: open.memory.imageUri }} style={styles.viewerImg} resizeMode="contain" />
            ) : null}
