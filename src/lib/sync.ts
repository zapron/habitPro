import type { Habit, HabitStore, MiniMission } from "../types/habit";
...
function miniToRow(userId: string, m: MiniMission) {
  return {
    id: m.id,
    user_id: userId,
    title: m.title,
...
export async function pushFullState(
  ...
  const miniRows = state.miniMissions.map((m) => miniToRow(userId, m));
  ...
  const { data: existingHabits } = await supabase
    ...
    .select("id")
    .eq("user_id", userId);
  const wantHabitIds = new Set(state.habits.map((h) => h.id));
  ...
  const { data: existingMini } = await supabase
    ...
    .select("id")
    .eq("user_id", userId);
  const wantMiniIds = new Set(state.miniMissions.map((m) => m.id));
...
export async function hydrateStoreAfterAuth(
  ...
  if (isRemoteEmpty(remote) && localHasData(local)) {
    apply(local);
    await pushFullState(userId, local);
    return;
  }
