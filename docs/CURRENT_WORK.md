...
Last updated: 2026-09-21 (end of session — growth Phase 1 shipped: invite-link sharing (habitPro-web + app), store review prompt, share-your-win card (3 new native modules, version bumped to 1.1.36, EAS builds triggered for Play Store + TestFlight). Also shipped request-to-join for group missions (squad-voted, mirrors streak-repair governance), a deep-link-resume fix, a real share-card photo bug found via user testing (fixed + OTA'd to production, update group `9da187d4-e45a-402d-8082-1828220ec336`), an Android build-version gotcha (stale gitignored `android/app/build.gradle`), and local Android dev-environment fixes. Full detail immediately below.
