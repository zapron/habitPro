...
Last updated: 2026-09-23 (end of session — Community got a real entry point into the group-mission request-to-join flow (tap a mission name in the feed or on a profile), the share-your-win card was redesigned (photo-forward + minimalist fallback, QR link embed), the Live Squad share-link was reverted (dead end, no request-to-join system there), a real badge-clipping bug and a real Community note-bleed-through bug were found and fixed, and local dev got a permanent fix for the recurring LAN-IP-drift "network request failed" issue. Full detail immediately below.
