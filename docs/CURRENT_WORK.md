...
Last updated: 2026-09-24 (a real production bug found and fixed: `rpc_get_community_access_status()` — the RPC the client calls to gate the Community paywall — only ever recognized `trial`-type grants, so a manually-granted `promo` premium extension was invisible to it and the paywall never cleared for that user. Also manually granted a real user 3 months of Community access via `community_access_grants` (the correct, non-billing-conflicting mechanism) after a support report. Full detail immediately below; prior session's entry (2026-09-23 — Community mission entry point, share-card redesign, misc fixes) follows after.
