...
Last updated: 2026-09-21 (end of session — growth Phase 1 shipped: invite-link sharing (habitPro-web + app), store review prompt, share-your-win card (3 new native modules, version bumped to 1.1.36, needs a real store build). Also shipped request-to-join for group missions (squad-voted, mirrors streak-repair governance), a deep-link-resume fix, and local Android dev-environment fixes. Full detail immediately below.
...
**Not yet done, explicitly next**: push this session's 3 commits (app) + `main` (habitPro-web, already pushed) to `origin`; user runs `db:push` for the new migration once ready; user builds and submits the new native binary via EAS/Xcode/Play Console themselves (**do not OTA this batch — the version bump exists specifically to prevent that**); the deferred Community habit-tap entry point is a clean next task whenever picked back up.
